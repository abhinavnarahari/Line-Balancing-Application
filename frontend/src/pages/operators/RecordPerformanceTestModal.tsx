  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          {benchmark && benchmark.rating5 && (
            <div className="flex items-center gap-1.5 flex-wrap text-[10px] bg-slate-50 border border-slate-200 p-2 rounded-xl font-mono mt-1.5 shadow-2xs">
              ...
              <span className="px-2 py-0.5 bg-[#FAF7F2] text-[#9C5B3C] border border-[#E6DDCE] rounded-md font-bold">⭐4: {benchmark.rating4?.label}</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              className="w-full h-9 border border-[#E6DDCE] rounded-lg px-3 text-xs text-[#221912] bg-white focus:outline-none focus:border-[#9C5B3C] focus:ring-1 focus:ring-[#9C5B3C]"
              required
            />
          </div>
          <div>
            <label className="block text-[11px] font-bold text-[#8C7E6E] uppercase tracking-wide mb-1">
              Recorded By
            </label>
            <input
              type="text"
        ...
        {/* Dynamic Test Runs */}
        <div className="bg-[#FAF7F2] p-4 rounded-xl border border-[#E6DDCE] space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 text-[11px] font-bold text-[#9C5B3C] hover:text-[#854D33] bg-white px-2.5 py-1 rounded-md border border-[#E6DDCE] hover:border-[#9C5B3C] shadow-2xs cursor-pointer transition-colors"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-h-48 overflow-y-auto pr-1">
            {testRuns.map((run, index) => (
              <div key={index} className="flex items-center gap-1.5 bg-white p-1.5 rounded-lg border border-[#E6DDCE] shadow-2xs">
                <span className="text-[10px] font-bold text-[#8C7E6E] pl-1 shrink-0">
                  #{index + 1}
                </span>
                <input
                  type="number"
                  ...
                  className="w-full h-7 px-1 text-xs font-semibold text-[#221912] focus:outline-none text-center bg-transparent"
                />
                {testRuns.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleRemoveRun(index)}
                    className="p-1 text-[#8C7E6E] hover:text-rose-600 rounded cursor-pointer"
                    title="Remove Run"
                  >
          ...
          {validTimes.length > 0 && derivedRating !== null && (
            <div className="mt-3 p-3 bg-white rounded-lg border border-[#E6DDCE] flex items-center justify-between flex-wrap gap-2 animate-in fade-in duration-200">
              <div className="flex items-center gap-2">
                <Calculator className="w-4 h-4 text-[#9C5B3C]" />
                <div>
                  <div className="text-[11px] text-[#8C7E6E]">
                    {validTimes.length} {validTimes.length === 1 ? "test" : "tests"} · Daily Average
                  </div>
                  <div className="text-sm font-bold text-[#221912]">
                    {avgTime.toFixed(1)}s
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-white bg-[#221912] px-2.5 py-1 rounded-md shadow-2xs">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full border border-[#E6DDCE] rounded-lg px-3 py-2 text-xs text-[#221912] bg-white focus:outline-none focus:border-[#9C5B3C] focus:ring-1 focus:ring-[#9C5B3C] resize-none"
          />
        </div>

        {/* Action Buttons: Save as Draft vs Submit */}
        <div className="flex items-center justify-between gap-2.5 pt-2 border-t border-[#F0EAE0]">
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-5 h-9 text-xs font-bold text-white bg-[#9C5B3C] hover:bg-[#854D33] rounded-lg disabled:opacity-50 flex items-center gap-1.5 cursor-pointer shadow-xs"
            >
