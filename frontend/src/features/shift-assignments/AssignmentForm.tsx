        ...
        <div className="flex flex-col space-y-1.5">
          ...
          <div className="relative mb-2">
            <input
              ...
              className="w-full h-8 text-[11px] bg-white border border-[#E6DDCE] rounded-sm pl-3 pr-3 text-[#221912] focus:outline-none focus:border-[#9C5B3C] focus:ring-1 focus:ring-[#9C5B3C]/20"
            />
          </div>
          <div className="h-40 overflow-y-auto bg-white border border-[#E6DDCE] rounded-sm p-2 space-y-1">
            {operators
              ...
              .map(op => {
                ...
                return (
                  <label key={op.id} className={`flex items-center justify-between gap-2 p-1.5 rounded cursor-pointer ${isAlreadyAssigned ? 'bg-[#FAF7F2] opacity-60 cursor-not-allowed' : 'hover:bg-[#FAF7F2]'}`}>
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="rounded-sm border-[#E6DDCE] text-[#9C5B3C] accent-[#9C5B3C] focus:ring-[#9C5B3C] disabled:opacity-50"
                      />
                      <span className="text-sm text-[#221912]">{op.employeeId} - {op.name}</span>
                    </div>
                    {isAlreadyAssigned && (
                      <span className="text-[10px] font-bold text-[#9C5B3C] uppercase tracking-wide">Already Assigned</span>
                    )}
                  </label>
                );
            })}
